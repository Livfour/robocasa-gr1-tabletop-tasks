...
from robocasa.models.scenes.scene_utils import initialize_fixture, load_style_config
import robocasa.utils.camera_utils as CamUtils
import robocasa.utils.object_utils as OU
import robocasa.models.scenes.scene_registry as SceneRegistry
from robocasa.models.scenes import TabletopArena
...
REGISTERED_TABLETOP_EVNS = {}


def register_tabletop_env(target_class):
    REGISTERED_TABLETOP_EVNS[target_class.__name__] = target_class


class TabletopEnvMeta(EnvMeta):
    """Metaclass for registering robocasa environments"""

    def __new__(meta, name, bases, class_dict):
        cls = super().__new__(meta, name, bases, class_dict)
        register_tabletop_env(cls)
        return cls
...
_ROBOT_POS_OFFSETS: dict[str, list[float]] = {
    ...
    "BDM1FloatingBody": [0, 0, 0.97],
    "BDM1": [0, 0, 0.97],
    "BDM1FixedLowerBody": [0, 0, 0.97],
    "BDM1ArmsOnly": [0, 0, 0.97],
...
class Tabletop(ManipulationEnv, metaclass=TabletopEnvMeta):
    """
    Initialized a Base Tabletop environment.
    ...
    Args:
        ...
        layout_and_style_ids (list of list of int): list of layout and style ids to use for the tabletop environment.

        layout_ids ((list of) TabletopLayoutType or int):  layout id(s) to use for the tabletop environment. -1 and None specify all layouts

        style_ids ((list of) StyleType or int): style id(s) to use for the tabletop. -1 and None specify all styles.
        ...
        prefer_leading_registry (bool): whether to bias the sampling towards the first registry in obj_registries
        ...
        distractor_config (dict): Configuration for distractor objects and fixtures. Format:
            {
                "regions": {
                    "back_edge": {  # Name of region
                        "placement": {
                            "pos": (0, 1),  # Position relative to table (-1 to 1)
                            "size": (1.2, 0.2),  # Size of region in meters
                        },
                        "fixtures": [  # List of fixtures to place in this region
                            {
                                "type": "mug_tree",  # Type of fixture
                                "count": (0, 3),  # Random number between 0-3 (optional, default is 1)
                            },
                            {
                                "type": "plant",
                                "count": 1,
                        ...
                        "objects": [  # List of objects to place in this region
                            {
                                "type": ["fruit", "vegetable"],  # Object type(s)
                                "count": (2, 4),
                            }
                        ]
                    },
        ...
        env_lang (str): kept for backwards compatibility
    """

    VALID_LAYOUTS = [0, 1, 2, 3, 4, 5]
    ...
    def __init__(
        ...
        obj_registries=("objaverse", "sketchfab", "lightwheel"),
        ...
        self.prefer_leading_registry = prefer_leading_registry
        ...
        self.layout_and_style_ids = [
            ...
            if l in self.VALID_LAYOUTS
        ]
        assert len(self.layout_and_style_ids) > 0

        assert generative_textures in [None, False, "100p"]
        self.generative_textures = generative_textures

        assert distractor_config is None or use_distractors
    ...
    def _load_model_monitor_wrapper(func):
        """Wrapper to count number of times _load_model is called"""

        def wrapper(self, *args, **kwargs):
            if not hasattr(self, "_load_model_count"):
                self._load_model_count = 0
            self._load_model_count += 1

            if self._load_model_count % 10 == 0:
                print(
                    f"WARNING: _load_model has been called over {self._load_model_count} times!"
                )
    ...
    @_load_model_monitor_wrapper
    def _load_model(self, reload=True):
        """
        ...
        if "layout_id" in self._ep_meta:
            ...
            layout_id, _ = self.rng.choice(self.layout_and_style_ids)
            self.layout_id = int(layout_id)

        # Set style_id
        if "style_id" in self._ep_meta:
            self.style_id = self._ep_meta["style_id"]
        else:
            _, style_id = self.rng.choice(self.layout_and_style_ids)
        ...
        # setup scene
        self.mujoco_arena = TabletopArena(
        ...
        if self.renderer == "mjviewer":
            camera_config = CamUtils.DEFAULT_LAYOUT_CAM
            self.renderer_config = {"cam_config": camera_config}

        # setup fixtures
        self.fixture_cfgs = self.mujoco_arena.get_fixture_cfgs()
        self.fixture_cfgs.extend(self._get_distractor_fixture_cfgs())
    ...
    def _get_distractor_fixture_cfgs(self):
        """Returns configurations for distractor fixtures to be placed in the scene"""
        if self.distractor_config is None or self.layout_id == 1:
            return []

        # Load style config
        style_path = SceneRegistry.get_style_path(self.style_id)
        with open(style_path, "r") as f:
            style = yaml.safe_load(f)

        processed_configs = []

        # Process each region's fixtures
        for region_name, region_config in self.distractor_config.get(
            "regions", {}
        ).items():
            placement = region_config.get("placement", {})

            for fixture_config in region_config.get("fixtures", []):
                count_spec = fixture_config.get("count", 1)
                if isinstance(count_spec, (list, tuple)):
                    min_count = count_spec[0]
                    max_count = count_spec[1]
                else:
                    min_count = max_count = count_spec

                for i in range(max_count):
                    cfg = {
                        "name": f"distractor_fixture_{region_name}_{fixture_config['type']}_{i}",
                        "type": fixture_config["type"],
                        "placement": {
                            **placement,
                    ...
                    if fixture_config.get("scale", None):
                        cfg["scale"] = fixture_config["scale"]

                    # Apply style configs
                    default_config = load_style_config(style, cfg)
                    for k, v in cfg.items():
                        default_config[k] = v
                    cfg = default_config

                    # Convert to fixture class and initialize
                    cfg["type"] = FIXTURES[cfg["type"]]
                    fixture = initialize_fixture(cfg, [], rng=self.rng)
                    cfg["model"] = fixture
                    cfg["type"] = "fixture"
    ...
    def _get_distractor_obj_cfgs(self):
        ...
        for region_name, region_config in self.distractor_config.get(
            ...
            for obj_config in region_config.get("objects", []):
                ...
                for i in range(max_count):
                    if isinstance(obj_config["type"], str):
                        obj_type = obj_config["type"]
                    else:
                        obj_type = self.rng.choice(obj_config["type"])
                    cfgs.append(
                        dict(
                            name=f"distractor_obj_{region_name}_{obj_type}_{i}",
                            type="object",
                            obj_groups=obj_type,
                            graspable=False,
                            placement={
    ...
    def _setup_table_references(self):
        """
        setup fixtures (and their references). this function is called within load_model function for tabletops
        """
    ...
    def compute_robot_base_placement_pose(self, offset=None):
        """
        steps:
        1. compute offset relative to the counter
        2. transform offset to global coordinates
        ...
        # get first counter in the environment
        base_fixtures = [
            fxtr for fxtr in self.fixtures.values() if isinstance(fxtr, Counter)
        ]

        # use first counter available
        base_fixture = base_fixtures[0]
    ...
    def _get_placement_initializer(self, cfg_list, z_offset=0.01):
        ...
        for obj_i, cfg in enumerate(cfg_list):
            ...
            if macros.SHOW_SITES is True:
                """
                show outer reset region in blue
                """
                ...
                """
                show inner reset region in red
                """
                pos_to_vis = deepcopy(ref_pos)
                pos_to_vis[:2] += T.rotate_2d_point(
            ...
            placement_initializer.append_sampler(
                sampler=UniformRandomSampler(
                    ...
                    ensure_object_in_ref_region=placement.get(
                        "ensure_object_in_ref_region", False
                    ),
                    ensure_object_out_of_ref_region=placement.get(
                    ...
                    num_attempts=100 if placement.get("optional", False) else 5000,
                ),
                sample_args=placement.get("sample_args", None),
                optional=placement.get("optional", False),
            )
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset and self.placement_initializer is not None:
            ...
            for obj_pos, obj_quat, obj in object_placements.values():
                for j in obj.joints:
                    # Reset free joints only
                    if np.size(self.sim.data.get_joint_qpos(j)) == 7:
                        self.sim.data.set_joint_qpos(
                            j, np.concatenate([np.array(obj_pos), np.array(obj_quat)])
                        )
    ...
    def set_cameras(self):
        ...
        if egoview_mount_body is not None:
            self._cam_configs["egoview"] = dict(
                pos=[
    ...
    def edit_model_xml(self, xml_str):
        ...
        for cam_name, cam_config in self._cam_configs.items():
            ...
            if cam is None:
                old_cam = find_elements(
                    root=worldbody, tags="camera", attribs={"name": cam_name}
                )
    ...
    def sample_object(
        ...
        """
        Sample a tabletop object from the specified groups and within max_size bounds.
        ...
        Args:
            ...
            exclude_cat (str or list): groups to exclude
        ...
        return sample_kitchen_object(
            ...
            prefer_leading_registry=self.prefer_leading_registry,
